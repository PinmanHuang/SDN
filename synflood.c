...
int main(int argc, char *argv[ ])
{
  ...
  struct sockaddr_in sin;	//處理網路address
  ...
  unsigned int floodport = atoi(argv[2]);
	...
	sin.sin_port = htons(floodport);	//dst port
	sin.sin_addr.s_addr = inet_addr(argv[1]);	//dst addr
	/* zero out the buffer */
	...
	char ipfirst[12] = "192.168.3.21";
	/*char iplast[10];
	srand(time(NULL));
	int iplastInt = rand()%256;
	sprintf(iplast, "%d", iplastInt); 
	strcat(ipfirst, iplast);
	printf("test : %s\n", ipfirst);*/
	iph->iph_sourceip = inet_addr (ipfirst);
	...
	/* number, and the ACK sequence is 0 in the 1st packet */		tcph->tcph_acknum = 0;
	tcph->tcph_res2 = 0;
	...
	int tmp = 1;
	const int *val = &tmp;
	if(setsockopt (s, IPPROTO_IP, IP_HDRINCL, val, sizeof (tmp)) < 0)	//interface option
	{
